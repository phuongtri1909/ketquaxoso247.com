...
use App\Http\Controllers\Frontend\DoVeOnlineController;
...
Route::controller(DuDoanXoSoController::class)->group(function () {
    ...
    Route::get('du-doan-xsmn-{date}', [DuDoanXoSoController::class, 'getDuDoanXSMNTheoNgay'])
        ->name('dudoan.xsmn.date')
        ->where(['date' => '^([0-9]{2}-[0-9]{2}-[0-9]{4})$', 'province_id' => '[0-9]+']);

    Route::get('du-doan-xsmt-{date}', [DuDoanXoSoController::class, 'getDuDoanXSMTTheoNgay'])
        ->name('dudoan.xsmt.date')
        ->where(['date' => '^([0-9]{2}-[0-9]{2}-[0-9]{4})$', 'province_id' => '[0-9]+']);
});
...
Route::controller(DoVeOnlineController::class)->group(function () {
    Route::get('/get-province/{date}','getProvince')->name('get.province');
    Route::get('/do-ve-so','doVeOnline')->name('do-ve-so');
});