        ...
        <div class="card">
            <div class="card-body">
                ...
                <form class="form-horizontal" role="form" enctype="multipart/form-data" method="POST"
                      action="{{route('cau-lo-de.update',$post->id)}}">
                    {{ csrf_field() }}
                    {{method_field('PATCH')}}
                        ...
                        <div class="col-xs-10">
                            <select name="type" class="form-control @error('type') is-invalid @enderror">
                                ...
                                @foreach($types as $value => $label)
                                    <option value="{{ $value }}" {{ (old('type', $post->type) == $value) ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="form-group">
                       ...
                       <textarea id="des" rows="5" name="des" placeholder="Mô tả"
                                 class="form-control @error('des') is-invalid @enderror">{!! old('des',$post->des) !!}</textarea>
                            @error('des')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-10">
                            <textarea rows="20" id="content" name="content" class="form-control my-editor @error('content') is-invalid @enderror">{{ old('content',$post->content) }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
