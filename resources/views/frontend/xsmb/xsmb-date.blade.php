        ...
        @if(!empty($xsmb))
            ...
            <div class="box" id='kqngay_{{getNgayID($xsmb->date)}}'>
                ...
                <div class="see-more">
                   ...
                   <p> <strong> <a href="{{route('xsmb.date',getNgayLink($date))}}" title="Xổ số miền Bắc XSMB{{getNgay($date)}}">Xổ số miền Bắc {{getNgay($date)}}</a> </li> - Kết quả xổ số miền Bắc hôm nay nhanh nhất tại xosotructiep.online Xổ số miền Bắc {{getNgay($date)}}</strong>
                    </p>
                    ...
                    <p>Kết quả xổ số miền Bắc XSMB{{getNgay($date)}} được công bố trên các trang web xosotructiep.online đề hoặc trên các kênh truyền hình vào mỗi ngày. </p>
