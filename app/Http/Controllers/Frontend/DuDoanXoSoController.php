...
//        $tkLoto = $tk_arr['tan_suat'];
        ...
        $content = str_replace('xosodaiphat.top','xosotructiep.online',$content);
   ...
   // MIỀN NAM
    ...
    {
        $ddXsmn = Post::where('category_id', 3)->orderBy('date', 'DESC')->where('province_id',null)->paginate(30);
        return view('frontend.dudoanxoso.du-doan-xsmn', compact('ddXsmn'));
    }

    public function getDuDoanXSMNTheoNgay($date, Request $request)
    {
        
        $dateYMD = getNgayLink($date);
        $province_slug = $request->get('province_slug');

        if($province_slug){
            $province = Province::where('slug_sc', $province_slug)->first();
            if(!$province || $province->mien != 3){
                return view('errors.404');
            }

            $dudoan = Post::where('category_id', 3)->where('date', $dateYMD)->where('province_id', $province->id)->orderBy('date', 'DESC')->first();
        }else{
            $dudoan = Post::where('category_id', 3)->where('date', $dateYMD)->where('province_id',null)->orderBy('date', 'DESC')->first();
        }
        if (empty($dudoan)) {
            return view('errors.404');
        }
...
//            $tkLoto[$pro->short_name] = $tk_arr['tan_suat'];
            ...
            $pro_custom = $pro;
            $content = view('frontend.dudoanxoso.du-doan-xsmn-theo-ngay-content',compact('date', 'dateYmd', 'xsmns','pascal','kqs','pro_custom'))->render();
            $content = str_replace('xosodaiphat.top','xosotructiep.online',$content);
            ...
            $title = "Dự đoán Xổ Số $pro_custom->name Miền Nam ngày $date - Dự đoán XSMN $dateDMY";

            Post::firstOrCreate([
                'link' => route('dudoan.xsmn.date', ['date' => getNgayLink($dateYmd), 'province_slug' => $pro_custom->slug_sc]),
    ...
    {
        $ddXsmt = Post::where('category_id', 2)->orderBy('date', 'DESC')->where('province_id',null)->paginate(30);
        return view('frontend.dudoanxoso.du-doan-xsmt', compact('ddXsmt'));
    }

    public function getDuDoanXSMTTheoNgay($date, Request $request)
    {
        ...
        if($province_slug){
            $province = Province::where('slug_sc', $province_slug)->first();
            if(!$province || $province->mien != 2){
                return view('errors.404');
            }

            $dudoan = Post::where('category_id', 2)->where('date', $dateYMD)->where('province_id', $province->id)->orderBy('date', 'DESC')->first();
        }else{
            $dudoan = Post::where('category_id', 2)->where('date', $dateYMD)->where('province_id',null)->orderBy('date', 'DESC')->first();
        }
    ...
    public function create_DuDoanXSMTTheoNgay_Content($day)
    {
        ...
        foreach ($province_mt as $pro) {
           ...
           $tkLoto[$pro->short_name] = $tk_arr['tan_suat'];
            ...
            $pro_custom = $pro;
            $content = view('frontend.dudoanxoso.du-doan-xsmt-theo-ngay-content',compact('date', 'dateYmd', 'xsmts','pascal','kqs','pro_custom'))->render();
            $content = str_replace('xosodaiphat.top','xosotructiep.online',$content);
            ...
            $title = "Dự đoán Xổ Số $pro_custom->name Miền Trung ngày $date - Dự đoán XSMT $dateDMY";
            Post::firstOrCreate([
                'link' => route('dudoan.xsmt.date', ['date' => getNgayLink($dateYmd), 'province_slug' => $pro_custom->slug_sc]),
